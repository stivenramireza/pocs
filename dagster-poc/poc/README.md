...
| `poc/`       | A Python directory that contains code for your Dagster repository                 |
| `poc_tests/` | A Python directory that contains tests for `poc`                      |
...
pyenv virtualenv $PYTHON_VERSION poc
pyenv activate poc
```
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name poc python=PYTHON_VERSION
conda activate poc
```
...
Tests can be found in `poc_tests` and are run with the following command:

```bash
pytest poc_tests
```

As you create Dagster ops and graphs, add tests in `poc_tests/` to check that your
